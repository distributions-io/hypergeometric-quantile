...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'm' ) ) {
		...
		if ( !isNonNegativeInteger( opts.m ) ) {
			return new TypeError( 'quantile()::invalid option. `m` parameter must be a non-negative integer. Option: `' + opts.m + '`.' );
		}
	}
	if ( options.hasOwnProperty( 'n' ) ) {
		opts.n = options.n;
		if ( !isNonNegativeInteger( opts.n ) ) {
			return new TypeError( 'quantile()::invalid option. `n` parameter must be a non-negative integer. Option: `' + opts.n + '`.' );
		}
	}
